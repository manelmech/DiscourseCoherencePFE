...
if( event.target.value==="option2"){
  ...
  const handleListModelsClick = (event, index) => {
    ...
    setSelectedIndex(modelNames.indexOf(Array.from(event)[Array.from(event).length - 1]))
  ...
  return (
    <>
        ...
        <div style={{ marginTop: '1%' }} >
          ...
          >
            <Item sx={{ backgroundColor: 'none', height: '50px', width: '100%' }}>
              <Typography variant="h5" sx={{ fontFamily: 'Poppins', fontWeight: 500, color: '#5885FB' }}><LooksOneIcon sx={{ margin: '0 18px', height: '6%', width: '6%', color: "#ffab00" }} /> {language=="fr"? "Insertion des données" : "Insert data"}</Typography>
            </Item>
        ...
        <Form onSubmit={handleSubmit} enctype="multipart/form-data">
          <div className='input_text'>
            <textarea
              ...
              required
              type='text'
              placeholder={language=="fr"? "Insérez un texte" : "Insert a text"}
              value={text}
          ...
          >
            <Item >
             ...
             <div className="file-inputs">
                <Button type="button" id='import_btn' onClick={handleClickOpenDialog}> {language=="fr"? fileName ?? "Importer un fichier": fileName ?? "Import a file"} </Button>
              </div>
          ...
          <div id="secondSection">
            <div>
              ...
              >
                <Item sx={{ backgroundColor: 'none', height: '50px', width: '100%' }}>
                  <Typography variant="h5" sx={{ fontFamily: 'Poppins', fontWeight: 500, color: '#5885FB' }}><LooksTwoIcon sx={{ margin: '0 18px', height: '5%', width: '5%', color: "#ffab00" }} />{language=="fr"? "Sélection du modéle" : "Model Selection"}</Typography>
                </Item>
              </Box>
            </div>
          </div>
      ...
      />
            ...
            <Button type="submit" id='eval_btn'>{language=="fr"? "évaluer " : "Evaluate"}</Button>
          </div>
      ...
      <Dialog  aria-labelledby="customized-dialog-title" open={openDialog} fullWidth='true' maxWidth='sm' PaperProps={{
       ...
       <div style={{margin:"10px  0px   10px 20%", padding:"10px", fontFamily: 'Poppins'}}>
                        <h4 >{language=="fr"? "Selectionnez une option" : "Select an option"}</h4>
       </div>
        <div style={{margin:"10px 25% 10px"}}>
        <p>{language=="fr"? "Importer un fichier .csv " : "Import a .csv file"}</p>
        <Button onClick={handleImportcsv}  variant="outlined" style={{ width: "250px",
        height: "50px" ,padding: "6px 10px"}} startIcon={<FileUploadOutlinedIcon/>}>{language=="fr"? "Importer un fichier " : "Import a file"}</Button>
        <input type="file" ref={hiddenFileInput} onChange={handleChange} style={{ display: 'none' }} />
  
        <br></br><br></br>
        <p>{language=="fr"? "Importer un fichier .txt " : "Import a .txt file"}</p>
        <Button onClick={handleImporttxt} variant="outlined" style={{ width: "250px",
        height: "50px" , marginBottom:"20px",padding: "6px 10px"}} startIcon={<FileUploadOutlinedIcon/>}> {language=="fr"? "Importer un fichier " : "Import a file"}</Button>
